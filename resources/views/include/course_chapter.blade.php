<div class="card btm-10">
                        ...
                        <td>
                            <div class="row">
                                <div class="col-lg-6 col-6">
                                    ...
                                    <div class="section">{{ __('Class') }}: <?php echo $i;?></div>
                                </div>
                                <div class="col-lg-6 col-6">
                                        ...
                                        @else
                                            ...
                                            ?>
                                                @if($coursechapter->goal_date>=$startDate)
                                                    ...
                                                    left</small><br>
                                                @else 
                                                <small class="text-danger">Your goal date is expired {{$coursechapter->goal_date}}</small><br>
                                                @endif
                                            @endif
                                        @endif
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-2 col-4">
                                    <div class="text-right">
                                        ...
                                        @if(isset($progress->mark_chapter_id) && in_array($coursechapter->id, $progress->mark_chapter_id))
                                            @if(isset($coursechapter->goal_date))
                                            Goal Date {{$coursechapter->goal_date}}
                                            @endif
                                        @else
    ...
    <div id="collapseChapter{{ $coursechapter->id }}" class="collapse" aria-labelledby="headingChapter" data-parent="#accordion">
        ...
        @foreach($coursechapter->courseclass->sortBy('position') as $class)
            ...
            @if(Auth::user()->role == "user" && $course->drip_enablte == 1 && $class->drip_type != NULL)
        ...
        @endforeach
        @php 
            $topics = App\QuizTopic::where('course_id', $course->id)->where('type', NULL)->where('chapter_id', $coursechapter->id)->first();
        @endphp
        @if ($topics) 
            @if ($coursechapter->id == $topics->chapter_id)
                @include('include.chapter_quiz')
            @endif
        @endif
